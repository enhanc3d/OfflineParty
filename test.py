...
def save_latest_post_data(artist: str, post_id: int, date: str, number_of_posts: int, last_downloaded_post: str, downloaded_posts: int):
    # This is your new save function, it now takes in the URL of the latest downloaded post and the total number of downloaded posts.
    data = {'post_id': post_id, 'date': date, 'number_of_posts': number_of_posts, 'last_downloaded_post': last_downloaded_post, 'downloaded_posts': downloaded_posts}
    all_data = {}
    ...
    if artist_id:
        ...
        for i, post_url in enumerate(new_post_urls):
            ...
            try:
                ...
                downloaded_posts += 1  # Increase the count of downloaded posts after a successful download
                ...
                if first_post:
                    save_latest_post_data(artist_id,
                                        ...
                                        post_date,
                                        number_of_posts,
                                        post_url,  # Save the URL of the latest downloaded post
                                        downloaded_posts)  # Save the current count of downloaded posts
                    first_post = False
                else:
                    save_latest_post_data(artist_id,
                                        post_id,
                                        post_date,
                                        number_of_posts,
                                        post_url,  # Update the URL of the latest downloaded post
                                        downloaded_posts)  # Update the current count of downloaded posts
            except Exception as e:
